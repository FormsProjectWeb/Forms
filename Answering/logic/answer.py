import openai
from .subject import topic, reference
from Forms.settings import OPEN_AI_KEY

openai.api_key = OPEN_AI_KEY

def answerRelatedTo(question, M=False):
    answer = openai.ChatCompletion.create(
        model = 'gpt-3.5-turbo',
        messages = [
        {
            'role': 'user',
            'content': prompt(topic, reference, question, M)
        }
        ],
        temperature = 0.3
    )
...
def prompt (topic, reference, question, M=False):
    if M:
        return f"""
        Basándote en el contexto de {topic}, y teniendo en cuenta que se encuentran principalmente los temas de: {reference}
